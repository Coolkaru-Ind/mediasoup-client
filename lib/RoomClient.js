...
import { getProtooUrl, getAPIUrl } from './urlFactory';
...
var apiUrl = getAPIUrl();//"https://134.122.126.57:4445/api/potentialcase"
...
export default class RoomClient {
	...
	async _joinRoom() {
		...
		// //var client = mqtt.connect('ws://test.mosquitto.org:8080')
		// var client = mqtt.connect('wss://test.mosquitto.org:8081')
		//ws://165.22.212.195:9001/
		console.log("sds")
		var roomclient = this;
		client.on('connect', function () {
			console.log("Connected")
			alert("Connected")
			client.subscribe('pulse/topic', function (err) {
				if (err) {
					console.log(`Error while subscribing to event ${err}`)
				}
				//client.publish('pulse/topic', '{"topic_name": "pulse/topic", "building_id": 1234588, "kisok-id": 784, "covid_status": true, "oxidation_level": 95, "temperature": 39, "image_url": "", "user_id": 1231, "timestamp": ""}')
		...
		client.on('message', function (topic, message) {
			// message is Buffer
			// important
			//let obj = { "thermometer": message.temperature, "oxymeter": message.oxidation_level }
			//console.warn(message.toString());
			//alert(message.toString())

			let mqttMessage = JSON.parse(message.toString());
			let messageToChar = {
				thermometer: Number(mqttMessage.temperature).toFixed(2),
				oxymeter: Number(mqttMessage.oxidation_level).toFixed(2)

			}
			console.warn("messageToChar", messageToChar)
			// const {  send } = this._chatDataProducer;
			//this._chatDataProducer.send(message.toString());
			roomclient.sendChatMessage(JSON.stringify(messageToChar));

			let objToAPI = {
				
					imageURL: mqttMessage.image_url.toString(),
					floor: "0",
					location: {
						top: "50%",
						right: "60%"
					...
					readings: {
						thermometer: Number(mqttMessage.temperature).toFixed(2).toString(),
						oxymeter: Number(mqttMessage.oxidation_level).toFixed(2).toString()
					},
					user_id: mqttMessage.user_id,
					ticketId: "testID1234"
				
			}
			console.warn(objToAPI);
			console.warn(JSON.stringify(objToAPI));
			...
			fetch(apiUrl, requestOptions)
				.then(async response => {
					console.warn("Server Responded", response)
					console.warn('https://134.122.126.57:4445/api/potentialcase post success')
				})
				.catch(error => {
					console.warn('https://134.122.126.57:4445/api/potentialcase there was an error', error);
				});
